...
void Reset_Handler(void)
{
    ...
    uint32_t size = &_edata - &_sdata; //Calculate the size of .data from memory location use &
    
    uint8_t *pDst = (uint8_t*)&_sdata; //SRAM
    uint8_t *pSrc = (uint8_t*)&_etext; //flash 
    for(uint32_t i=0; i< size; i++)
    {
      *pDst++ = *pSrc++;   //copy data from flash to SRAM
    }
    ...
    for(uint32_t i=0; i< size; i++)
    {
      *pDst++ = 0;   //copy data from flash to SRAM
    }
